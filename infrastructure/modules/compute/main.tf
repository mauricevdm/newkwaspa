# Compute Module
# EC2 Spot Fleet, Auto Scaling Group, Application Load Balancer
...
resource "aws_launch_template" "magento" {
  name_prefix   = "${var.name_prefix}-magento-"
  ...
  tag_specifications {
    ...
    tags = merge(var.tags, {
      Name = "${var.name_prefix}-magento-volume"
    })
  }
...
resource "aws_lb_target_group" "magento" {
  name     = "${var.name_prefix}-magento-tg"
  port     = 80
  protocol = "HTTP"
  vpc_id   = var.vpc_id
...
resource "aws_autoscaling_group" "magento" {
  name                = "${var.name_prefix}-magento-asg"
  ...
  mixed_instances_policy {
    instances_distribution {
      on_demand_base_capacity                  = var.use_spot_instances ? 0 : var.desired_capacity
    ...
    launch_template {
      launch_template_specification {
        launch_template_id = aws_launch_template.magento.id
        version            = "$Latest"
      }
